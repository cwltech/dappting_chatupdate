...
class ChatHomeList extends StatefulWidget {
  final type;

  const ChatHomeList({Key? key, this.type}) : super(key: key);

  @override
  State createState() => HomePageState();
}

class HomePageState extends State<ChatHomeList> {
  ...
  Widget build(BuildContext context) {
    context.read<UserProvider>().userChatList(user_id.toString());
    return Scaffold(
      appBar: AppBar(
        elevation: 0,
                ...
                Expanded(
                  child: Consumer<UserProvider>(
                    builder: (context, state, child) {
                      return state.map["data"]["UserList"] != null
                          ? ListView.builder(
                              itemCount: state.map["data"]["UserList"].length,
                              itemBuilder: (BuildContext context, int index) {
                                return ListTile(
                                  ...
                                  title: Text(
                                    state.map["data"]["UserList"][index]
                                        ["name"],
                                    style: const TextStyle(
                                        color: Colors.black,
                                        fontWeight: FontWeight.bold),
                                  ),
                                  subtitle: Text(state.map["data"]["UserList"]
                                      [index]["last_message"]),
                                  ...
                                  trailing: Text(state.map["data"]["UserList"]
                                          [index]["user_id"]
                                      .toString()),
                                );
                              })
